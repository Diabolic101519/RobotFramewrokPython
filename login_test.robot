...
${LOGIN_FORM}     xpath://*[@id="login_field"]
${PASSWORD_FIELD}    xpath://*[@id="password"]
${LOGIN_BUTTON}   xpath://button[@type='submit'] | //input[@type='submit']
${GOOGLE_BUTTON}  xpath://button[contains(normalize-space(.), 'Continue with Google')] | //a[contains(normalize-space(.), 'Continue with Google')]
...
Unsuccessful Login Scenario
    ...
    [Teardown]    Close All Browsers
    Go To    ${LOGIN_URL}
    Wait Until Page Contains Element    ${LOGIN_FORM}    timeout=20s
...
Continue With Google Scenario
    ...
    [Teardown]    Close All Browsers
    Go To    ${LOGIN_URL}
    Wait Until Page Contains Element    ${GOOGLE_BUTTON}    timeout=20s
...
Submit Credentials
    [Arguments]    ${user}    ${pass}
    Input Text        ${LOGIN_FORM}    ${user}
    Input Password    ${PASSWORD_FIELD}    ${pass}
    Wait Until Page Contains Element    ${LOGIN_BUTTON}    timeout=20s
    Click Element    ${LOGIN_BUTTON}

Successful Login Should Be Confirmed
    ${login_failed}=    Run Keyword And Return Status
    ...    Wait Until Page Contains    ${LOGIN_ERROR}    timeout=10s
...
Unsuccessful Login Should Be Confirmed
    Wait Until Page Contains    ${LOGIN_ERROR}    timeout=20s
